...
export default function VendorsPage() {
  ...
  const onSubmit = async (values: VendorFormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ 
          error: editingVendor ? 'Failed to update vendor' : 'Failed to create vendor' 
        }));
        throw new Error(errorData.error || (editingVendor ? 'Failed to update vendor' : 'Failed to create vendor'));
      }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input 
                      ...
                      {...field} 
                      className="p-3 bg-white text-gray-800 border-gray-300" 
                    />
                  </FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-800 font-semibold">GST Number *</FormLabel>
                  <FormControl>
                    <Input 
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-800 font-semibold">Contact *</FormLabel>
                  <FormControl>
                    <Input 
            ...
            <div className="pt-4">
              ...
              <div className="mb-4">
                <label className="block text-gray-800 font-semibold mb-2">Search Address</label>
                <AddressAutocomplete
                  onAddressSelect={handleAddressSelect}
                  defaultValue={editingVendor?.address || ''}
                />
                <p className="text-sm text-gray-600 mt-1">Enter an address above, then edit the fields below as needed</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-800 font-semibold">Street/Location</FormLabel>
                      <FormControl>
                        <Input 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-800 font-semibold">Pincode</FormLabel>
                      <FormControl>
                        <Input 
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-800 font-semibold">Google Maps Link</FormLabel>
                      <FormControl>
                        <div className="flex">
                          <Input 
                            placeholder="Enter Google Maps URL" 
                            {...field} 
                            className="p-3 bg-white text-gray-800 border-gray-300 rounded-r-none" 
                          />
                          {field.value && (
                            <a 
                              href={field.value} 
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="mt-4">
                    <FormLabel className="text-gray-800 font-semibold">Full Address (Legacy)</FormLabel>
                    <FormControl>
                      <Input 
            ...
            <div className="flex justify-end pt-6">
              ...
              >
                {form.formState.isSubmitting ? 'Saving...' : (editingVendor ? 'Update Vendor' : 'Save Vendor')}
              </Button>
            </div>
          </form>
        </Form>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                vendors.map(vendor => (
                  <TableRow key={vendor.id} className="hover:bg-gray-50 border-b border-gray-200">
                    <TableCell className="text-gray-800">{vendor.id}</TableCell>
                    <TableCell className="font-medium text-gray-800">{vendor.name}</TableCell>
                    <TableCell className="text-gray-800">{vendor.gstNo || '-'}</TableCell>
                    <TableCell className="text-gray-800">{vendor.contact || '-'}</TableCell>
                    <TableCell className="text-gray-700">
                      ...
                      {!vendor.street && !vendor.city && !vendor.state && !vendor.pincode && vendor.address}
                      {vendor.maps_link && (
                        <a 
                    ...
                    <TableCell>
                      <Button variant="outline" size="sm" className="text-blue-600 border-blue-200 hover:bg-blue-50" onClick={() => handleEdit(vendor)}>
                        Edit
                      </Button>
                    </TableCell>
                  </TableRow>
